...
#script original vibe:  https://github.com/mkocabas/VIBE
#to make it work, you need a SMPL unity file
...
def process_poses(
    ...
    # altera para Easymocap

    #easymocap_path=r'D:\MOCAP\EasyMocap-master\Teste_20210328_1_out\smpl'
    easymocap_path= input_path
    #s_list = glob.glob(easymocap_path+'\\'+'*.json')
    s_list = glob.glob(easymocap_path+'*.json')
    # append_data = pd.DataFrame(pd.DataFrame.from_dict(json.load(open(s_list[0],'r'))[0]['poses']))
    data = []
    append_data = pd.DataFrame(columns = range(72)) #criando empty dataframe com as colunas
    for i in s_list:
        data = pd.DataFrame(pd.DataFrame.from_dict(json.load(open(i,'r'))[0]['poses']))
